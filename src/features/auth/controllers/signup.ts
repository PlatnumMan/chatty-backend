...
export class SignUp {
  ...
  public async create(req: Request, res: Response): Promise<void> {
    ...
    if (!result?.public_id) {
      throw new BadRequestError('Avatar image upload failed');
    }

    // Add to redis cache

    const userDataForCache: IUserDocument = SignUp.prototype.userData(authData, userObjectId);
    userDataForCache.profilePicture = `https://res.cloudinary.com/dqudzggrb/image/upload/v${result.version}/${userObjectId}`;
  ...
  private signupData(data: ISignUpData): IAuthDocument {
    ...
    return {
      _id,
      uId,
      username: Helpers.firstLetterUppercased(username),
